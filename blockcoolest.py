...
surface_size = 1000
main_surface = pygame.display.set_mode((900,900))
...
def draw_tree(inord, order, theta, thetab, sz, posn, heading, color=(0,0,0), depth=0):
   ...
   gradd  = 128+(127*math.cos((heading*pygame.mouse.get_pos()[0]/400).imag))
   gradd2 = 128+(128*math.sin((heading*1j*pygame.mouse.get_pos()[1]/400).real))
    ...
    def callback(in_data, frame_count, time_info, status):
       ...
       if math.fabs(decoded[0]) > 0:
           if math.fabs(decoded[1]) > 0:
               #print(decoded[0])
               for x in decoded:
                  theta1 = math.sin((math.exp((math.fabs(x)/6000)*1 )))*1j
                  theta2 = math.sin((math.exp((math.fabs(x)/6000)*1)))*1j
                  draw_tree(inord, 2, theta1, theta2, surface_size*2, (450,450), math.pi)
       #pygame.display.flip()
